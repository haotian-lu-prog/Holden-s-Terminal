...
const pageRenderers: Record<PageId, (state: TerminalState) => string> = {
...
function loadState(): TerminalState {
  ...
  try {
    const parsed = JSON.parse(stored) as Partial<TerminalState>;
    if (parsed.schemaVersion !== 2) return fallback;
    return {
      ...fallback,
      ...parsed,
...
function countFor(page: PageId): string {
...
function bindEvents(): void {
  app.querySelectorAll<HTMLAnchorElement>("[data-nav]").forEach((anchor) => {
  ...
  app.querySelectorAll<HTMLButtonElement>("[data-action='export']").forEach((button) => {
    button.addEventListener("click", exportData);
  });

  app.querySelectorAll<HTMLButtonElement>("[data-action='choose-import']").forEach((button) => {
    button.addEventListener("click", () => {
      app.querySelector<HTMLInputElement>("[data-action='import']")?.click();
    });
  });

  app.querySelectorAll<HTMLInputElement>("[data-action='import']").forEach((input) => {
...
async function importData(event: Event): Promise<void> {
  ...
  try {
    const imported = JSON.parse(await file.text()) as Partial<TerminalState>;
