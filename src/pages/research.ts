...
import type { ResearchStatus, ResearchTopic, TerminalState } from "../types/investment.js";

const statusOrder: ResearchStatus[] = ["idea", "researching", "active thesis", "closed"];

export function renderResearch(state: TerminalState): string {
  const tags = tagInventory(state.researchTopics);
  const averageConviction =
    state.researchTopics.reduce((sum, topic) => sum + topic.conviction, 0) / state.researchTopics.length;
  ...
  return `
    ${SectionHeader({
      ...
      description:
        "Track topic tags, thesis state, linked tickers, conviction, and the current bull/bear debate.",
...
function tagInventory(topics: ResearchTopic[]): [string, number][] {
